					...
					board.columns
						...
						.map(async (column: any) => {
							...
							switch (columnType) {
								...
								case 'timeline':
									// Timeline needs special handling - it's an object with from/to
									type = 'string';
									break;
			...
			try {
				if (resource === 'item') {
					if (operation === 'create') {
						...
						// Handle different mapping modes from resourceMapper
						if (columnsData.mappingMode === 'autoMapInputData') {
							...
							// Manual mapping - columnsData.value contains the field mappings
